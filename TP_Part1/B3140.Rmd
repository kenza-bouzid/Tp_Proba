...
Il faut noté que les courbes suivantes représentent un cas particulier résultant de la génération aléatoire des dates de départ et d'arrivée, nous nous sommes basés sur plusieurs simulations afin de juger de manière pertinente le comportement des files d'attente en fonction de ces paramètres. 
Afin d'alléger le compte rendu, nous nous limitons à deux simulations par paramètres. 
...
On voit que pour un $\alpha$ très proche de 1, contrairement à l'exemple précédent. Nous avons beaucoup de mal à prédire le comportement de la courbe. Le modèle est beaucoup plus soumis à l'aléatoire. Le nombre de client en attente peut monter très haut sur une durée très longue et redescendre abrutement dans certains cas. Mais on peut aussi avoir un cas ou le nombre de clients augmente et descend très rapidement sur des petits intervalles de temps. Ceci est du au fait qu'on soit à la limite séparant la stabilité de la sturation du sytème. 
...
* Simulation 3:$\lambda$ = 20 et $\mu$ = 15
...
On a ici un * $\alpha$ supérieur à 0. C'est à dire qu'on plus de personnes arrivant dans la file que de personne partant de la file. Le nombre de personnes croît continuellement, on a donc ici une courbe prédictible qui va systématiquement diverger, on atteint la saturation assez 

### Vérification de la formule de Little

Nous nous intéressons dans cette partie à l'étude de régime stationnaire. Nous étudierosn ainsi différentes simulations dont l'intensité du traffic est inférieur à 1 afin de garantir le caractère stable de la simulation et pourvoir appliquer les différentes approximations. 
Il s'agira ainsi d'estimer le nombre moyen de clients dans le système ainsi que le temps de présence d'un client dans le sytème. 
Le nombre moyen de clients peut être obtenu en calculant la moyenne du nombre de clients sur l'intervalle de temps étudié. Cette moyenne peut être obtenu en calculant la somme totale des clients représentée par la surface sous la courbe de simulation puis en divisant par l'intervalle d'étude. Comme nous ne disposons pas de la fonction décrivant l'évolution du nombre de clients pendant le temps, nous sommes incapable de calculer cette surface grâce à une integrale, cependant nous arrivons à approximer cette surface grâce à une somme de riemanne. Nous considérons que nos intervalles de temps sont assez fin afin de garantir un minimum de justesse et de finess à nos calculs. 
...
Un fois ces nombres déterminés, nous vérifierons si ces valeurs s'approchent des valeurs théoriques en comparant le nombre de clients moyen à la valeur $\alpha/\alpha-1$ et en vérifiant la validité de la formule de Little. 

Un exemple de simulation étant généré aléatoirement ne suffit pas pour conclure, nous calculerons ainsi ces différents taux pour plusieurs simulations de $\alpha$ respectant l'hypothèse de système stationnaire. Nous réaliserons nos calculs pour des intervalles de temps plus larges afin de minimiser les portentielles erreurs. 

Nous présentons notre étude dans le tableau suivant: nous avons réalisé 100 simulations différentes pour chaque valeur de $\lambda$ afin d'obtenir une meilleure précision en minimisant les erreurs éventuelles dûes à l'aléatoire.
...
Ainsi pour $\alpha=0.53$ nous obtenons une nombre de client moyen de `r liste1[[1]]` ce qui est tout à fait cohérent avec la courbe d'évolution où l'on observait 1 client très fréquemment. 
Pour les deux valeurs de $\lambda$, nous avons calculé le temps obtenu selon la formuyle de Little, on remarque un écart théorique très faible plus important dans le premier cas. On en conclut qu'on vérifie bien la formule de Little pour les deux valeurs de $\lambda$. 
Ceci peut nous amener à confirmer encore une fois le régime stationnaire atteint par la file d'attente pour $\lambda=8$, cependant on peut rien conclure de plus pour  $\lambda=14$ malgrè qu'elle vérifie la formule de Little. En effet,sa courbe d'évolution représente un caractère pas tout à fait stationnaire comme nous l'avons précisé auparavant. 

On pourrait également essayer d'augmenter l'intervalle d'étude pour rafiner les calculs et voir à quel poin t on peut se rapprocher de la formule de Little. Le tableau suivant, récapitule les résulatas obtenus pour une même valeur de  $\lambda=8$, 100 simulations et une période de temps variable allant de 24 à 200 heures. 
...
***
On remarque qu'on gagne faibement en précision, on peut ainsi se resteindre à un nombre raisonnable d'heures afin d'affirmer nos observations. 

Finalement on pourrait explorer d'autres valeurs de $\lambda$ afin d'évaluer à quel point on se rapproche de la formule de Little pour différentes intensité de traffic. 

Nous obtenons le graphique suivant représentant l'évolution du temps de présence Théorique et le temp de Little pour différentes valeurs de $\lambda$. 
