...
Nos premiers jeux de tests étudié les séquences en elles même en visualisant leur distribution et leur inter-dépendance. Nous nous interessons dans ce qui suit à ces séquences en étudiant celles-ci bit à bit. Cela nous permettra d'évaluer le caractère aléatoire de ces différentes séquences avec plus de finesse. Il s'agira d'alligner toutes les représentation binaires des différentes séquences côte à côte et d'étudier le caractère aléatoire des valeurs que prennent ces différents bits.Ceci a pour intérêt de partiellement déterminer le caractère aléatoire du générateur étudié. 
...
On note Sn la séquence globale résulatant de la concaténation binaire des différentes séquences. Chacun des bits de cette séquence binaire suit ainsi une loi de Bernouilli. Ainsi la somme de ces bits suit une loi binomiale.  
Selon le théorème de la limite centrale $Sn/sqrt(n)$ suit une loi normale centrée réduite. 

Nous étudirons ainsi la loi de probalité suit par $|Sn|/sqrt(n)$, Si celle ci est inférieure à 1% nous affirmons selon la règle de décision à 1% que celle-ci ne suit pas notre modèle de loi normale #contradiction, notre hypothèse n'est donc pas juste et la séquece ne présnete pas un caractère aléatoire. Si par contre on obtient des probas assez élevés nous affirmerons notre hypothèse et jugerons celle-ci comme étant bien aléatoire. 
...
Pour 100 tests différents, on observe pour Von Neumann, une fréquence extrêmement élevé pour des Pvaleurs très petites. C'est à dire que dans la majorité des cas, Von Neumann obtient une Pvaleur en dessous dudit 1%. 
...
On note aussi que la discontinuité aurait pu être comblée au faisant les tests sur un nombre supérieur de valeurs. 
...
Ain d'exploiter les résultats de ce test, nous nous sommes limités à évaluer la fréquence es pvaleurs ainsi que leur répartition par rapport à la limite 1% qu'on représente encore une fois par une fdroite en rouge. 
...
Nous concluons à l'issu de ses différents tests statistiques que le caractère aléatoire du générateur Von Neumann est très limité par rapport aux autres générateurs. LE générateur Randu s'est montré aléatoire à l'issu du premier et dernier test mais nous restons très douteux de son pouvoir à garantir un caractère tout à fait pour les différentes graines générées. Mersenne Twister et Standard Minimum ont compte à eux réussi tous les tests.
